...
public abstract class HeldItemRendererMixin {
    ...
    private float shakeBowBasedOnArrowDivergence(float y_scale, @Local(argsOnly = true) @NotNull AbstractClientPlayerEntity player) {
        return y_scale * (player.getProjectileType(Items.BOW.getDefaultStack()).getItem() instanceof CustomArrowItem arrow ?arrow.divergence : 1.0f);
    }
    ...
    private float scaleBowBasedOnChargeTime(float tick_rate_idk, @Local(argsOnly = true) @NotNull AbstractClientPlayerEntity player) {
        return 20.0f * (player.getProjectileType(Items.BOW.getDefaultStack()).getItem() instanceof CustomArrowItem arrow ? arrow.charge_time : 1.0f);
    }
}
